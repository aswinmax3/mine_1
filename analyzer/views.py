...
def upload_policy(request):
    if request.method == 'POST':
        ...
        if request.user.email:
            try:
                send_verification_email(request.user, doc)
...
def policy_chat(request, pk):
    doc = get_object_or_404(PolicyDocument, pk=pk, user=request.user)
...
def multi_policy_ask(request):
    ...
    docs = PolicyDocument.objects.filter(user=request.user).order_by('-uploaded_at')
    answer = ask_multi_policy_question(docs, question) or 'Sorry, I could not find an answer.'
    return JsonResponse({'answer': answer})
...
def premium_calculator(request):
    if request.method == 'POST':
        ...
        return render(request, 'analyzer/premium_result.html', {
            'result': result,
            'data': data
        })
...
def enter_project(request):
    return render(request, "analyzer/enter_project.html")
